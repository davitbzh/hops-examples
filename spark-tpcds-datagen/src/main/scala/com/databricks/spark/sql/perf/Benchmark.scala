...
//                  .save(s"hdfs:///Projects/benchmark/benchmark_Training_Datasets/selected_tpcds/$name.parquet")
    ...
    def writehtml: Unit ={
      ...
      val output_stream = filesystem.create(new Path(s"$resultsLocation/${currentExecution}=$timestamp.html"));
      val buffered_output = new BufferedOutputStream(output_stream)
      buffered_output.write(html.getBytes(StandardCharsets.UTF_8))
