 ...
 */
trait TPCDS_1_4_Queries_hoodie extends Benchmark {
  ...
  /*
  discovery/Interactive (ad-hoc) - q19, q42, q52, q55, q63, q68, q73, q98
  ...
  val tpcds1_4Queries = Seq(
    ("q1", s"""
             ...
             |    AND DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1
    ...
    ("q2", s"""
             ...
             |        FROM web_sales WHERE DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1 ) x
             |        UNION ALL
             |       (SELECT cs_sold_date_sk sold_date_sk, cs_ext_sales_price sales_price
             |        FROM catalog_sales WHERE DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1 )),
             | wswscs AS
             | (SELECT d_week_seq,
    ...
    ("q9", s"""
              ...
              |from reason
              |where r_reason_sk = 1 AND DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1
            """.stripMargin),
    ("q10", s"""
              | select
              ...
              |  AND DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1 AND
    ...
    ("q12", s"""
              ...
              | where
              | DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1 AND
    ...
    ("q13", s"""
              ...
              |     ,customer_address
              |     ,date_dim
              | where DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1 AND s_store_sk = ss_store_sk
    ...
    ("q14a", s"""
               ...
               |       AND DATEDIFF(_hoodie_commit_time, $timeStamp) BETWEEN -1 and 1 ) x
